...
import { getBankOfficerComplaintsThunk } from "../../../store/features/bankOfficer/thunk/bankOfficerComplaintThunk";
import type { ComplaintType } from "../../../store/features/bankOfficer/bankOfficerComplaintSlice";
...
const ComplaintBankOfficer = () => {
  ...
  const { paginated, loading } = useSelector(
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      ...
      try {
        await dispatch(getBankOfficerComplaintsThunk(query)).unwrap();
      } catch (error: any) {
        ...
        if (isUnauthorized) {
          ...
          try {
            ...
            dispatch(getBankOfficerComplaintsThunk(query));
          } catch (refreshError) {
    ...
    <Box sx={{ minHeight: "100vh", bgcolor: "#f8fafc", pb: 8 }}>
      ...
      <Container maxWidth="lg" sx={{ mt: -7 }}>
            ...
            <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
              ...
              <FormControl sx={{ flex: 1 }}>
                ...
                >
                  ...
                  {banks
                    .filter((b) => b._id === user?.bankId) // filter by user's bankId
                    .map((b) => (
                      <MenuItem key={b._id} value={b.bankCode}>
                        {b.bankName}
                      </MenuItem>
                    ))}
                </Select>
        ...
        {loading ? (
          ...
          <Stack spacing={2.5}>
            <AnimatePresence>
              {paginated?.complaints?.map(
                (complaint: ComplaintType, idx: number) => (
                  <motion.div
                    key={complaint._id}
                  ...
                  >
                    <Card
                      sx={{
                        ...
                        transition: "0.3s",
                        "&:hover": {
                          boxShadow: "0 12px 24px rgba(0,128,128,0.08)",
                    ...
                    >
                      <CardActionArea
                        onClick={() =>
                          navigate(
                            `/dashboard/bank/complaints/${complaint._id}`,
                          )
                        }
                        sx={{ p: 1 }}
                      >
                        <CardContent>
                          ...
                          >
                            ...
                            <Chip
                              ...
                              sx={{
                                ...
                                fontSize: "0.75rem",
                                bgcolor:
                                  statusColor[complaint.status]?.bg ||
                                  "#f8fafc",
                                color:
                                  statusColor[complaint.status]?.text ||
                                  "#64748b",
                          ...
                          >
                            ...
                            <Stack
                              ...
                              spacing={1}
                            >
                              {(complaint?.remark?.length ?? 0) > 0 && (
                                <Button
                                  size="small"
                                  variant="contained"
                                  onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                                    e.stopPropagation(); // 🔥 prevent CardActionArea navigation
                                    navigate(
                                      `/dashboard/bank/complaints/${complaint._id}/remarks`,
                                    );
                                  }}
                                  sx={{
                                    textTransform: "none",
        ...
        {!loading && paginated && paginated?.totalPages > 1 && (
          <Box sx={{ display: "flex", justifyContent: "center", mt: 6 }}>
            <Pagination
              ...
              size="large"
              sx={{
                "& .Mui-selected": { bgcolor: PRIMARY_TEAL, color: "white" },
              }}
            />
          </Box>
        )}
...
export default ComplaintBankOfficer;
