...
import { bankOfficerGetAllUsersThunk } from "../../../store/features/bankOfficer/thunk/bankOfficer.thunk";
...
const AdminUsersDashboard = () => {
  ...
  const user = useSelector((state: RootState) => state.bankOfficer.user);
  ...
  //set bank filter
  useEffect(() => {
    if (!isAdmin && user?.bankId && banks?.length) {
  ...
  const loadData = useCallback(async () => {
    if (!bankFilter) return; // only call if bankFilter exists
    try {
      const usersRes = await dispatch(
        bankOfficerGetAllUsersThunk({
          page: 1,
          bankCode: bankFilter,
          search,
          status: statusFilter,
  ...
  }, [dispatch, bankFilter, search, statusFilter]); // banks remove karo
        ...
        >
          ...
          >
            ...
            >
              ...
              {banks?.map((b: any) => (
                <MenuItem key={b._id} value={b.bankCode} sx={{ color: "#000" }}>
                  {b.bankCode}
                </MenuItem>
              ))}
            </TextField>
            ...
            >
              ...
              <MenuItem value="active" sx={{ color: "#000" }}>
                Active
              </MenuItem>
              <MenuItem value="inactive" sx={{ color: "#000" }}>
                Inactive
              </MenuItem>
            </TextField>
          </Box>
